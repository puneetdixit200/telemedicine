...
const referralSummarySchema = z.object({
  appointmentId: appointmentIdSchema,
  targetSpecialty: z.string().trim().max(140).optional().or(z.literal('')),
  referralReason: z.string().trim().min(3).max(1200),
  urgency: z.enum(['emergency', 'urgent', 'within_7_days', 'routine']).default('within_7_days'),
  triedTreatment: z.string().trim().max(1200).optional().or(z.literal('')),
  language: nonEmptyLanguage.optional().or(z.literal(''))
});
