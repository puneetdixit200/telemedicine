...
const documentsController = {
  ...
  downloadLink: async (req, res, next) => {
    try {
      ...
      const url = getReadSasUrl({
        blobName: doc.blobName,
        expiresInMinutes: 10,
        contentDisposition: buildContentDisposition(doc.fileName, 'attachment'),
  ...
  previewPdf: async (req, res, next) => {
    try {
      ...
      if (!String(doc.contentType || '').toLowerCase().includes('pdf')) {
        return res.status(415).json({ error: 'Preview supports PDF files only.' });
      }

      const url = getReadSasUrl({
        blobName: doc.blobName,
        expiresInMinutes: 10,
        contentDisposition: buildContentDisposition(doc.fileName, 'inline'),
